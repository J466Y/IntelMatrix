...
def upload_file():
    ...
    if data_type == "credentials":
        ...
        cmd = [python_cmd, "import.py", "-f", filepath, "-d", leak_date, "-n", leak_name, "-t", "misc"]
    else:
...
# --------------------------------------------------------------------------
# Leak Search (API-based)
# --------------------------------------------------------------------------

@app.route('/leaksearch', method='GET')
@view('views/leaksearch.tpl')
def leaksearch():
    if not is_authenticated():
        return redirect('/')
    ...
    if not target:
        return dict(target='', target_type='', report=None)

    target = escape(target)
    report = leak_search.lookup_report(target)
    target_type = leak_search._detect_type(leak_search._normalize_target(target))
    logging.info("Leak search completed for target '%s' (type: %s)", target, target_type)
    return dict(target=target, target_type=target_type, report=report)


@app.route('/leaksearch/export', method='GET')
def leaksearch_export():
    if not is_authenticated():
        return redirect('/')
    ...
    writer.writerow(['source', 'date_compromised', 'stealer_family', 'computer_name',
                     'operating_system', 'ip', 'total_corporate_services',
                     'total_user_services', 'top_logins', 'top_passwords'])
    for s in report.hr_stealers:
        writer.writerow([
            'hudson_rock', s.date_compromised, s.stealer_family,
            s.computer_name, s.operating_system, s.ip,
            s.total_corporate_services, s.total_user_services,
            ' | '.join(s.top_logins), ' | '.join(s.top_passwords)
        ])

    # Employees / Users
    if report.hr_employees or report.hr_users:
        writer.writerow([])
        writer.writerow(['type', 'url', 'occurrence'])
        for emp in report.hr_employees:
            writer.writerow(['employee', emp.get('url', ''), emp.get('occurrence', '')])
        for usr in report.hr_users:
            writer.writerow(['user', usr.get('url', ''), usr.get('occurrence', '')])
    ...
    if report.comb_entries:
        ...
        for entry in report.comb_entries:
            writer.writerow(['proxynova_comb', entry.line, entry.email, entry.password])

    # LC sources
    if report.lc_sources:
        writer.writerow([])
        writer.writerow(['source', 'name', 'date'])
        for src in report.lc_sources:
            writer.writerow(['leakcheck', src.name, src.date])

    csv_bytes = output.getvalue().encode('utf-8')
    response.content_type = 'application/force-download; charset=UTF-8'
    slug = re.sub(r'[^a-zA-Z0-9._-]', '_', target)[:50]
    response.set_header('Content-Disposition', f'attachment;filename=leaksearch_{slug}.csv')
    return csv_bytes
...
@app.route('/hostname-check', method='GET')
@view('views/hostname_check.tpl')
def hostname_check():
    if not is_authenticated():
        return redirect('/')

    hostname = request.query.get('hostname', '').strip()
    if not hostname:
